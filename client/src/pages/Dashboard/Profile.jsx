...
function Profile() {
  ...
  const handleSaveUsername = async () => {
    if (!newUsername.trim()) {
      toast.error("Username cannot be empty", options);
      return;
    }
    try {
      const { data } = await axios.post(`${updateUsernameRoute}/${user._id}`, {
        username: newUsername.trim()
      });
      if (data.status) {
        toast.success("Username updated successfully", options);
        const updatedUser = { ...user, username: data.user.username };
        localStorage.setItem('codebyte-user', JSON.stringify(updatedUser));
        setUser(updatedUser);
        setIsEditingUsername(false);
        setForceRerender(prev => !prev);
      } else {
        toast.error(data.msg || "Error updating username", options);
      }
    } catch (err) {
      console.error(err);
      toast.error("Error updating username. Please try again.", options);
    }
  };
  const options = {
            ...
            <div>
              {isEditingUsername ? (
                <div className="edit-username-container">
                  ...
                  />
                  <button className="save-btn" onClick={handleSaveUsername} title="Save">
                    <Check size={18} color="#22c55e" />
                  </button>
                  <button className="cancel-btn" onClick={() => { setIsEditingUsername(false); setNewUsername(user.username); }} title="Cancel">
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                  ...
                  <button className="edit-icon-btn" onClick={() => setIsEditingUsername(true)} title="Edit Username">
                    <Edit size={16} color="#AFAFAF" />
                  </button>
                </div>
              )}
              <h5>{user.email}</h5>
...
.profilebox{
  ...
  .edit-icon-btn, .save-btn, .cancel-btn {
    background: none;
    border: none;
    cursor: pointer;
    display: flex;
